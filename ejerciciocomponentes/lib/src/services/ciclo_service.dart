...
class CicloService {
  Future<List<Ciclo>> cargarTodosLosCiclos() async {
    final response = await http.get(
        Uri.parse('http://10.0.2.2:5000/api/Ciclo/Obtener_Todos_Los_Ciclos'));
  ...
  Future<List<Ciclo>> cargarCiclosPorFamiliaYTipo(
    ...
    final response = await http.get(Uri.parse(
        'http://10.0.2.2:5000/api/Ciclo/Obtener_Ciclos_Por_Familia_Y_Tipo?idTipo=' +
            idTipo +
            '&idFamilia=' +
            idFamilia));
    if (response.statusCode == 200) {
  ...
  Future<List<Ciclo>> cargarCiclosPorTipo(String idTipo) async {
    final response = await http.get(Uri.parse(
        'http://10.0.2.2:5000/api/Ciclo/Obtener_Ciclos_Por_Tipo?idTipo=' +
            idTipo));
    if (response.statusCode == 200) {
      return cicloFromJson(response.body);
    } else {
  ...
  Future<List<Ciclo>> cargarCiclosPorFamilia(String idFamilia) async {
    final response = await http.get(Uri.parse(
        'http://10.0.2.2:5000/api/Ciclo/Obtener_Ciclos_Por_Familia?idFamilia=' +
            idFamilia));
    if (response.statusCode == 200) {
      return cicloFromJson(response.body);
    } else {
